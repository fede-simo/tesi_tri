...
\paragraph{RunpodOllamaProvider}
...
La logica del sistema di generazione alert è organizzato in varie componenti.

\subsubsection{Rules}
Le rules permettono di effettuare ricerche al database periodicamente per ricercare delle violazioni delle regole settate. La maggior parte delle regole non sono generali ma 
permettono all'utente di settare parametri opzionali per personalizzare le notifiche che questo ha necessità ricevere. 
\newline
Queste, come i tool forniscono un interfaccia base \texttt{AlertRuleContract} che fornisce il contratto per tutte le sottoclassi figlie, una classe astratta \texttt{AlertRule} 
che fornisce un implementazione per i metodi comuni (in questo caso i metodi che permettono alla rule di accedere ai parametri settati dall'utente e che gestiscono i permessi di
utilizzo).
\newline
Infine si ha l'implementazione vera e propria della regola, contenente i metadati utilizzati per la UI e i metodi \texttt{check}, utilizzato per effettuare le query a database
per trovare le violazioni della stessa e \texttt{toAlert}, la quale formatta le informazioni ricevute e ritorna un \acrfull{dto} che verrà poi adattato in un alert.
\newline 
Le regole alert implementate rispondono a varie esigenze degli utenti finali di \textit{ErpAI} e sono state implementate per poter rispondere tempestivamente a queste esigenze.
Alcuni esempi comprendono \textit{LateBuyOrdersRule} e \textit{LateInvoicesRule} le quali ritornano notifiche riguardanti gli ordini (di acquisto o di vendita rispettivamente) in
ritardo rispetto alle aspettative, \textit{SchedulingErrorsRule} la quale segnala errori e discrepanze nei prodcessi di scehduling della produzione, oppure 
\textit{PausedMachineRule} la quale ritorna le macchine che non stanno eseguendo alcuna lavorazione, compreso di tempistiche e causa della pausa.


\subsection{Sistema di generazione degli alert}
...
\subsubsection{Resources e Pages}
