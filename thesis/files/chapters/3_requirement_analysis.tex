...
\rowcolors{2}{lightblue}{white}
    \begin{longtable}{|l|p{0.55\textwidth}|l|}
        \hline
        \rowcolor{gold}
        \textbf{Codice} & \textbf{Descrizione requisito} & \textbf{UC Relativo} \\
        \hline
        \endhead
        ...
        \textbf{RFN-1} & Il sistema deve permettere all'utente amministratore di configurare il modulo XAI & UC1 \\
        \hline
        \textbf{RFN-2} & Il sistema deve permettere all'utente amministratore di selezionare il modello LLM utilizzato dal modulo XAI & UC1.1  \\
        \hline
        \textbf{RFN-3} & Il sistema deve permettere all'utente amministratore di configurare gli utenti autorizzati ad utilizzare il modulo XAI & UC1.2  \\
        \hline
        \textbf{RFN-4} & Il sistema deve permettere all'utente amministratore di configurare i tool disponibili al modulo XAI & UC1.3  \\
        \hline
        \textbf{RFN-5} & Il sistema deve permettere all'utente di richiedere al modulo XAI una spiegazione relativa a una decisione presa dal gestionale & UC2\\
        \hline
        \textbf{RFN-6} & Il sistema deve permettere all'utente di creare, consultare e continuare conversazioni con il modulo XAI & UC2.1 \\
        \hline
        \textbf{RFN-7} & Il modulo XAI deve poter effettuare chiamate ai tool disponibili per recuperare le informazioni necessarie alla generazione delle spiegazioni & UC2.2 \\
        \hline
        \textbf{RFN-8} & Il sistema deve gestire l'indisponibilità del modello LLM, informando l'utente dell'impossibilità di elaborare la richiesta & UC2.3  \\
        \hline
        \textbf{RFN-9} & Il sistema deve gestire il superamento del limite di token durante l'elaborazione di una richiesta al modulo XAI, informando l'utente dell'impossibilità di completare la richiesta & UC2.4 \\
        \hline
        \textbf{RFN-10} & Il sistema deve impedire agli utenti privi dei permessi necessari di accedere alle informazioni richieste tramite il modulo XAI & UC2.5  \\
        \hline
        \textbf{RFN-11} & Il sistema deve permettere all'utente di richiedere la creazione di nuovi record tramite il modulo XAI & UC3  \\
        \hline
        \textbf{RFN-12} & Il modulo XAI deve poter effettuare chiamate ai tool necessari alla creazione dei record richiesti dall'utente & UC3.1  \\
        \hline
        \textbf{RFN-13} & Il sistema deve verificare la correttezza e la validità dei dati forniti per la creazione di un nuovo record e informare l'utente in caso di dati non validi & UC3.2 \\
        \hline
        \textbf{RFN-14} & Il sistema deve permettere all'utente di configurare le regole per la generazione degli alert & UC4  \\
        \hline
        \textbf{RFN-15} & Il sistema deve permettere all'utente di visualizzare le notifiche relative agli alert generati & UC5  \\
        \hline
        \textbf{RFN-16} & Il sistema deve gestire il superamento della soglia massima di violazioni rilevate, evitando la generazione di un numero eccessivo di notifiche e informando l'utente dell'evento & UC5.1  \\
        \hline
        \textbf{RFN-17} & Il sistema deve permettere all'utente di visualizzare il dettaglio di un alert generato & UC6  \\
        \hline
        \textbf{RFN-18} & Il sistema deve permettere all'utente di visualizzare il modello associato all'alert che ha causato la violazione della regola & UC7  \\
        \hline
        \textbf{RFN-19} & Il sistema deve permettere all'utente di richiedere al modulo XAI una spiegazione relativa all'alert generato & UC8 \\
        \hline
        \textbf{RFN-20} & Il sistema deve permettere all'utente di visualizzare l'elenco delle operazioni registrate nel sistema & UC9 \\
        \hline
        \textbf{RFN-21} & Il sistema deve permettere all'utente di visualizzare i dettagli di un'operazione registrata, compresi i dati relativi alle modifiche effettuate & UC10\\
        \hline
    \end{longtable}
...
\rowcolors{2}{lightblue}{white}
    \begin{longtable}{|l|p{0.42\textwidth}|p{0.2\textwidth}|l|}
        ...
        \textbf{RQN-1} & Il sistema deve garantire tempi di risposta adeguati nell'interazione con il modulo XAI, evitando attese non necessarie da parte dell'utente & UC1 - UC3.2, UC8\\
        \hline
        \textbf{RQN-2} & Il sistema deve garantire la sicurezza delle informazioni, impedendo agli utenti di accedere a dati per i quali non possiedono i permessi necessari & UC1 - UC10 \\
        \hline
        \textbf{RQN-3} & Il sistema deve garantire la tracciabilità delle operazioni effettuate sui dati, registrando creazione, modifica ed eliminazione dei modelli & UC9 - UC10  \\
        \hline
        \textbf{RQN-4} & Le spiegazioni generate dal modulo XAI devono essere comprensibili per l'utente e presentate in linguaggio naturale & UC1 - UC3.2, UC8 \\
        \hline
        \textbf{RQN-5} & Il sistema deve garantire tempi di risposta adeguati durante il recupero dei dati e nella conseguente generazione e notifica degli alert & UC5, UC5.1 \\
        \hline
        \textbf{RQN-6} & Il sistema deve notificare tempestivamente l'utente in caso di eccezioni nell'uso di modulo XAI e alert per ridurre le attese inutili & UC2.3 - UC2.5, UC5.1 \\
    \end{longtable}
...
\rowcolors{2}{lightblue}{white}
    \begin{longtable}{|l|p{0.42\textwidth}|l|l|}
        ...
        \textbf{RVN-1} & Il sistema deve essere sviluppato in PHP & - \\
        \hline
        \textbf{RVN-2} & L'interfaccia grafica deve essere sviluppata in Filament & -\\
        \hline
        \textbf{RVN-3} & Il modulo XAI deve fornire le spiegazioni generate in lingua italiana & UC1 - UC3.2, UC8\\
        \hline
        \textbf{RVN-4} & L'integrazione con i modelli linguistici deve avvenire attraverso le API messe a disposizione dai relativi provider & UC1 - UC3.2, UC8\\
        \hline
    \end{longtable}

Si precisa che tutti i requisiti individuati durante l'attività di analisi sono considerati \textbf{obbligatori} e, di conseguenza, devono essere necessariamente 
soddisfatti.
\newpage