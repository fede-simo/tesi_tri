...
\rowcolors{2}{lightblue}{white}
    \begin{longtable}{|l|p{0.55\textwidth}|l|}
        ...
        \textbf{RFN-1} & Il sistema deve permettere all'utente amministratore di configurare il modulo XAI & Sì \\
        \hline
        \textbf{RFN-2} & Il sistema deve permettere all'utente amministratore di selezionare il modello LLM utilizzato dal modulo XAI & Sì  \\
        \hline
        \textbf{RFN-3} & Il sistema deve permettere all'utente amministratore di configurare gli utenti autorizzati ad utilizzare il modulo XAI & Sì  \\
        \hline
        \textbf{RFN-4} & Il sistema deve permettere all'utente amministratore di configurare i tool disponibili al modulo XAI & Sì  \\
        \hline
        \textbf{RFN-5} & Il sistema deve permettere all'utente di richiedere al modulo XAI una spiegazione relativa a una decisione presa dal gestionale & Sì\\
        \hline
        \textbf{RFN-6} & Il sistema deve permettere all'utente di creare, consultare e continuare conversazioni con il modulo XAI & Sì \\
        \hline
        \textbf{RFN-7} & Il modulo XAI deve poter effettuare chiamate ai tool disponibili per recuperare le informazioni necessarie alla generazione delle spiegazioni & Sì \\
        \hline
        \textbf{RFN-8} & Il sistema deve gestire l'indisponibilità del modello LLM, informando l'utente dell'impossibilità di elaborare la richiesta & Sì  \\
        \hline
        \textbf{RFN-9} & Il sistema deve gestire il superamento del limite di token durante l'elaborazione di una richiesta al modulo XAI, informando l'utente dell'impossibilità di completare la richiesta & Sì \\
        \hline
        \textbf{RFN-10} & Il sistema deve impedire agli utenti privi dei permessi necessari di accedere alle informazioni richieste tramite il modulo XAI & Sì  \\
        \hline
        \textbf{RFN-11} & Il sistema deve permettere all'utente di richiedere la creazione di nuovi record tramite il modulo XAI & Sì  \\
        \hline
        \textbf{RFN-12} & Il modulo XAI deve poter effettuare chiamate ai tool necessari alla creazione dei record richiesti dall'utente & Sì  \\
        \hline
        \textbf{RFN-13} & Il sistema deve verificare la correttezza e la validità dei dati forniti per la creazione di un nuovo record e informare l'utente in caso di dati non validi & Sì \\
        \hline
        \textbf{RFN-14} & Il sistema deve permettere all'utente di configurare le regole per la generazione degli alert & Sì  \\
        \hline
        \textbf{RFN-15} & Il sistema deve permettere all'utente di visualizzare le notifiche relative agli alert generati & Sì  \\
        \hline
        \textbf{RFN-16} & Il sistema deve gestire il superamento della soglia massima di violazioni rilevate, evitando la generazione di un numero eccessivo di notifiche e informando l'utente dell'evento & Sì  \\
        \hline
        \textbf{RFN-17} & Il sistema deve permettere all'utente di visualizzare il dettaglio di un alert generato & Sì  \\
        \hline
        \textbf{RFN-18} & Il sistema deve permettere all'utente di visualizzare il modello associato all'alert che ha causato la violazione della regola & Sì  \\
        \hline
        \textbf{RFN-19} & Il sistema deve permettere all'utente di richiedere al modulo XAI una spiegazione relativa all'alert generato & Sì \\
        \hline
        \textbf{RFN-20} & Il sistema deve permettere all'utente di visualizzare l'elenco delle operazioni registrate nel sistema & Sì \\
        \hline
        \textbf{RFN-21} & Il sistema deve permettere all'utente di visualizzare i dettagli di un'operazione registrata, compresi i dati relativi alle modifiche effettuate & Sì\\
        \hline
    \end{longtable}
